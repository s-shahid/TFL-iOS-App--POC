...
- (void)prepareFrameSize {
	...
	frame.size = CGSizeMake(self.mapView.frame.size.width-130, height-10);
	self.frame = frame;
}
...
- (void)adjustMapRegionIfNeeded {
	...
	if (xPixelShift || yPixelShift) {
		...
		self.centerOffset = CGPointMake(self.centerOffset.x - xPixelShift, self.centerOffset.y - yPixelShift);
	}
}
...
//						 rect.origin.y + rect.size.height);
    ...
    CGPathAddLineToPoint(path, NULL, parentX-55, 
						 rect.origin.y + rect.size.height);
	CGPathAddLineToPoint(path, NULL, parentX-67, 
						 rect.origin.y + rect.size.height + 15);
    CGPathAddLineToPoint(path, NULL, parentX -80 , 
						 rect.origin.y + rect.size.height);
	...
	UIImage *toolTipImage = [UIImage imageNamed:@"tool_tip.png"];
   	color = [[UIColor colorWithPatternImage:toolTipImage] colorWithAlphaComponent:.9];
	[color setFill];
    ...
    UIImage *toolTipImages = [UIImage imageNamed:@"tool_tip.png"];
   	color = [[UIColor colorWithPatternImage:toolTipImages] colorWithAlphaComponent:.9];
	[color setStroke];
